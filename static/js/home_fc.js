...
function initialize(){
  ...
  files_obj = new Vue({
    ...
    methods :{
      ...
      click: function(items){
        if(info_obj.file.hash == items.hash){
          return
        }
        info_obj.file = items
        if(items.his.split('/n').length>1){
          info_obj.his_arrow = "fas fa-long-arrow-alt-down"
        }else{
          info_obj.his_arrow = ""
        }
        if(items.tag.length>1){
          info_obj.tag_arrow = "fas fa-long-arrow-alt-down"
        }else{
          info_obj.tag_arrow = ""
        }
        ...
        if(type=='image'){
          ...
          document.getElementById('audio').src = '/get_media/'+info_obj.file.hash
      ...
      //雙擊事件
      //針對folder物件
      //當folder被雙擊就進入該folder
      dbclick: function(items){
  ...
  info_obj = new Vue({
    el: '#file_info',
    delimiters : ['{[' , ']}'],
    data: {file: {id:-1, name:'', size:-1, tag:[], his:''}, his_arrow:"", tag_arrow:""}
  });
...
function format_speed(speed){
  ...
  return Math.round(speed).toString()+UNIT[t]+'/s';
}