...
class AoiMobilePlayer{
  constructor(id,set_html=true){
    ...
    */
    //時間軸
    this.bar = new DraggableBarMobile(
      '_bar',{
        //滑鼠按在滑條上
        //暫停後將時間跳到該處
        mousedown: function (this_bar, event, p){
    ...
    */
    ...
    this.video.addEventListener(
      'click', function(event){
        var d = new Date()
        if(self.video.paused){
          self.last_click = d.getTime()
          self.change_playing_state()
          return 
        }
        if(d.getTime()-self.last_click < 2000){
          self.change_playing_state()
        }
        self.last_click = d.getTime()
      }
  ...
  change_playing_state(){
    ...
    //暫停=>撥放
      ...
      window.setTimeout(()=>{
        if((d.getTime()-self.last_move)==0 && !self.video.paused){
          self.control_panel.style.opacity = '0'
          self.root.style.cursor = "none";
        }
      },2000)
